...
def haversine(coord1, coord2):
    lon1, lat1, lon2, lat2 = map(np.radians, [coord1[0], coord1[1], coord2[0], coord2[1]])
    dlon = lon2 - lon1
    dlat = lat2 - lat1
...
def nearest_neighbor_variant(dist_matrix, start_index, end_index):
    ...
    while len(path) < n - 1:  # Visit all but the last node (customer)
        next_city = np.argmin([dist_matrix[current][j] if not visited[j] and j != end_index else float('inf') for j in range(n)])
...
def calculate_shortest_distance(locations):
    ...
    return "Total Distance: {:.2f} km".format(total_distance)
